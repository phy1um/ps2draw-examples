...
int gs_init(struct gs_settings *cfg) {
  ...
  fb.psm = cfg->psm;
  fb.address =
      graph_vram_allocate(cfg->width, cfg->height, cfg->psm, GRAPH_ALIGN_PAGE);
  used_vram_bytes =
      graph_vram_size(cfg->width, cfg->height, cfg->psm, GRAPH_ALIGN_PAGE);
  ...
  zbuf.zsm = cfg->psmz;
  zbuf.address =
      graph_vram_allocate(cfg->width, cfg->height, cfg->psmz, GRAPH_ALIGN_PAGE);
  used_vram_bytes +=
      graph_vram_size(cfg->width, cfg->height, cfg->psmz, GRAPH_ALIGN_PAGE);

  // Setup video mode and graphics
  int vmode = cfg->mode == GRAPH_MODE_AUTO ? graph_get_region() : cfg->mode;
  int gmode =
      cfg->interlaced ? GRAPH_MODE_INTERLACED : GRAPH_MODE_NONINTERLACED;
  ...
  printf("Setup GS [[ VMode=%d, Interlace=%d, Width=%d, Height=%d ]]\n", vmode,
         gmode, cfg->width, cfg->height);

  // Send init to GS
  qword_t *buf = static_buffer;
  qword_t *q = draw_setup_environment(buf, 0, &fb, &zbuf);
  q = draw_primitive_xyoffset(q, 0, 2048 - (cfg->width / 2),
                              2048 - (cfg->height / 2));
  q = draw_finish(q);
 ...
 */
int gs_get_fbaddr() { return fb.address; }

/**
 * Poor abstraction, temporary
 */
zbuffer_t *gs_get_zbuf() { return &zbuf; }
...
int gs_clear(drawbuf *b, struct colour *rgb) {
  ...
  qws = draw_clear(qws, 0, 2048.0f - halfw, 2048.0f - halfh, fb.width,
                   fb.height, 0x0, 0x00, 0x00);
  // qws = draw_enable_tests(qws, 0, gs_get_zbuf());
  dma_channel_send_normal(DMA_CHANNEL_GIF, static_buffer, qws - static_buffer,
                          0, 0);
  dma_wait_fast();
  return 1;
}
